...
DsJwtAuth.checkToken = async function _checkToken() {
  let tempTokenExpirationTimestamp = null;
  if(tokenExpirationTimestamp) tempTokenExpirationTimestamp = moment(tokenExpirationTimestamp);
  
  let noToken = !DsJwtAuth.accessToken || !tokenExpirationTimestamp
    , now = moment()
    , needToken = noToken || tempTokenExpirationTimestamp.add(
